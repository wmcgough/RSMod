...
class SQLService : PlayerSerializerService()
{
    ...
    override fun initSerializer(server: Server, world: World, serviceProperties: ServerProperties) {
        ...
        // Configuration
        val driver = serviceProperties.getOrDefault("driver", "org.postgresql.Driver")
        val port   = serviceProperties.getOrDefault("port", 5432)
        val host   = serviceProperties.getOrDefault("host", "localhost")
        val user   = serviceProperties.getOrDefault("user", "postgres")
        val pswd   = serviceProperties.getOrDefault("password", "3eb92644")
        val dbname = serviceProperties.getOrDefault("dbname", "postgres")
